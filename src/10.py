...
def part_one() -> int:
    ...
    return sum(Chunk.corrupted_character_value.get(corrupted_character) for line in ls if (corrupted_character := get_corrupted_character(line)))
...
def part_two() -> int:
    with open("src/10.txt") as file:
        ls = file.read().splitlines()
    scores: list[int] = sorted(get_completion_string_score(completion_string) for line in ls if (completion_string := get_completion_string(line)))
